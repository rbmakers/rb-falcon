...
class NRF24L01:
    def __init__(self, spi, cs, channel=46, payload_size=16):
...
def responder():
    ...
    while True:
        if nrf.any():
            while nrf.any():
                buf = nrf.recv()
                #millis, millis2, led_state = struct.unpack("iii", buf)
                xValue, yValue = struct.unpack("ii", buf)
                print("received:", xValue, yValue)
